...
var import_obsidian5 = require("obsidian"), ObsidianLiveSyncSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    C.createEl("div", "", (s => s.innerHTML = `<a href='https://github.com/${T}/blob/main${I}' target="_blank">Open in browser</a>`));
    const R = C.createEl("div", {
      ...
      await import_obsidian.MarkdownRenderer.render(this.plugin.app, `<a class='sls-troubleshoot-anchor'></a> [Tips and Troubleshooting](${I}) [PageTop](${_})\n\n${E}`, R, `${A}`, this.plugin);
...
var StorageEventManager = class {}, StorageEventManagerObsidian = class extends StorageEventManager {
  ...
  async appendWatchEvent(s, r) {
    ...
    if (this.plugin.settings.isConfigured) if (!this.plugin.settings.suspendFileWatching) for (const u of s) {
      ...
      this.plugin.fileEventQueue.enqueueWithKey(`file-${E.path}`, {
        type: g,
        args: {
          file: E,
          oldPath: m,
...
async function putDBEntry(s, r) {
  ...
  if (D.length) {
    ...
    Logger(`Chunks saved: doc: ${g} ,chunks: ${_} (new:${r.ok.length}, recycled:${r.skip.length}, cached:${k})`);
  }
  const C = {
  ...
  return null != (o = await serialized("file:" + u, (async () => {
    try {
      const r = await s.localDatabase.get(C._id);
      C._rev = r._rev;
    } catch (s) {
      if (isErrorOfMissingDoc(s)) ; else throw s;
    }
    const r = await s.localDatabase.put(C, {
      force: true
    });
  ...
  async syncInternalFilesAndDatabase(s, r, o = false, u = false) {
    ...
    if (("pull" == s || "pullForce" == s) && 0 != D) if (m in k) {
      ...
      try {
        ...
        for (const r of u) if (r.dir && r.dir in k) {
          s -= k[r.dir];
    ...
    const S = [], L = await async function fetchChanges() {
      ...
      try {
        ...
        for await (const L of o.findAllNormalDocs()) {
          ...
          let k;
          const C = (null !== (s = (await o.getRaw(L._id, {
            revs_info: true
          }))._revs_info) && void 0 !== s ? s : []).reverse(), T = -1, I = 0, A = 1;
          for (const s of C) if ("available" == s.status) {
            const C = !D && g || m && s.rev == L._rev ? await o.getDBEntry(O, {
              rev: s.rev
            }, false, false, true) : await o.getDBEntryMeta(O, {
              rev: s.rev
            }, true);
            if (false === C) continue;
            const R = s.rev, x = "mtime" in C ? C.mtime : 0;
            if (b > x) continue;
            if (E < x) continue;
            let P = "";
            if (g && !D) {
              const s = getDocData(C.data);
              if (void 0 === k) k = s;
              if (k != s) {
                const r = new import_diff_match_patch.diff_match_patch, o = r.diff_main(k, s);
            ...
            const N = C._deleted || (null == C ? void 0 : C.deleted) || false;
            if (N) P += " 🗑️";
            if (R == L._rev) if (m) {
              const s = u.vaultAccess.getAbstractFileByPath(stripAllPrefixes(u.getPath(L)));
              if (s instanceof import_obsidian.TFile) {
                const r = await u.vaultAccess.adapterReadAuto(s), o = readAsBlob(C);
                if (await isDocContentSame(r, o)) P += " ⚖️"; else P += " ⚠️";
              }
            }
            const B = u.getPath(C), [F, ...M] = B.split("/").reverse();
            let V = "";
            if (_) {
              const s = null !== (r = null == C ? void 0 : C.children) && void 0 !== r ? r : [], u = await o.allDocsRaw({
                keys: [ ...s ]
            ...
            S.push({
              ...
              size: null !== (w = null == C ? void 0 : C.size) && void 0 !== w ? w : 0,
              isDeleted: N,
              changes: P,
              chunks: V,
              isPlain: D
...
var SETTING_HEADER = "````yaml:livesync-setting\n", SETTING_FOOTER = "\n````", ObsidianLiveSyncPlugin = class extends import_obsidian.Plugin {
  constructor() {
    ...
    this.databaseQueuedProcessor = new QueueProcessor((async s => {
      ...
      } else if (isValidPath(this.getPath(g))) this.storageApplyingProcessor.enqueueWithKey(g.path, g); else Logger(`Skipped: ${g._id.substring(0, 8)}`, LOG_LEVEL_VERBOSE); else Logger(`Something went wrong while gathering content of ${u} (${o._id.substring(0, 8)}, ${null == (r = o._rev) ? void 0 : r.substring(0, 10)}) `, LOG_LEVEL_NOTICE);
    }), {
      suspended: true,
      batchSize: 1,
      concurrentLimit: 10,
    ...
    this.storageApplyingProcessor = new KeyedQueueProcessor((async s => {
      ...
      Logger(`Processing ${g} (${u._id.substring(0, 8)}: ${null == (r = u._rev) ? void 0 : r.substring(0, 5)}) :Started...`, LOG_LEVEL_VERBOSE);
      ...
      if (_ instanceof import_obsidian.TFolder) Logger(`${this.getPath(u)} is already exist as the folder`); else {
        ...
        Logger(`Processing ${g} (${u._id.substring(0, 8)} :${null == (o = u._rev) ? void 0 : o.substring(0, 5)}) : Done`);
      }
    }), {
      suspended: true,
      batchSize: 1,
    ...
    this.conflictCheckQueue = new QueueProcessor((s => {
      const r = s[0];
      if (!(this.vaultAccess.getAbstractFileByPath(r) instanceof import_obsidian.TFolder)) return [ {
        key: r,
        entity: {
          filename: r
        }
  ...
  async loadQueuedFiles() {
    if (this.settings.suspendParseReplicationResult) return;
    if (!this.settings.isConfigured) return;
    const s = "obsidian-livesync-queuefiles-" + this.getVaultName(), r = arrayToChunkedArray([ ...new Set(JSON.parse(localStorage.getItem(s) || "[]")) ], 100);
    for await (const s of r) {
      ...
      this.replicationResultProcessor.enqueueAll(r.rows.map((s => s.doc)));
      await this.replicationResultProcessor.waitForPipeline();
    }
  }
  parseReplicationResult(s) {
    if (this.settings.suspendParseReplicationResult) this.replicationResultProcessor.suspend();
    this.replicationResultProcessor.enqueueAll(s);
    if (!this.settings.suspendParseReplicationResult) this.replicationResultProcessor.resume();
  }
  async realizeSettingSyncMode() {
    var s;
    this.replicator.closeReplication();
  ...
  observeForLogs() {
    const s = reactive((() => {
    ...
    })), r = reactive((() => 0 != this.requestCount.value - this.responseCount.value ? "📲 " : "")), o = reactive((() => {
      ...
      const s = this.pendingFileEventCount.value, r = this.processingFileEventCount.value, o = s - r;
      return `${0 != r ? `⏳${r} ` : ""}${0 != o ? ` 🛫${o}` : ""}`;
    })), g = reactive((() => {
      const {w: g, sent: _, pushLast: m, arrived: b, pullLast: E} = o.value, w = s.value, S = u.value;
      return {
        message: `${r.value}Sync: ${g} ↑${_}${m} ↓${b}${E}${S} ${w}`
      };
    })), _ = reactive((() => {
      const {message: s} = g.value;
      return {
        message: s,
  ...
  async syncAllFiles(s) {
    ...
    const L = [], O = s ? LOG_LEVEL_NOTICE : LOG_LEVEL_INFO, runAll = async (s, r, o) => {
      if (0 == r.length) {
      ...
      const _ = new QueueProcessor((async r => {
        ...
        if ((u + g) % 10 == 0) Logger(`${s}: DONE:${u}, FAILED:${g}, LAST:${_._queue.length}`, O, `log-${s}`);
      }), {
        batchSize: 1,
        concurrentLimit: 10,
        delay: 0,
      ...
      Logger(`${s} All done: DONE:${u}, FAILED:${g}`, O, `log-${s}`);
    };
    L.push(runAll("UPDATE DATABASE", b, (async s => {
    ...
    L.push(runAll("UPDATE STORAGE", E, (async s => {
    ...
    const D = new QueueProcessor((async s => {
      const r = s[0];
      return [ {
        file: r,
    ...
    D.pipeTo(new QueueProcessor((async s => {
      const r = await this.localDatabase.allDocsRaw({
        keys: s.map((s => s.id)),
        include_docs: true
